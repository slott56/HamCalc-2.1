...
def hours_to_sexagesimal():
    ...
    hrs= float( hrs_raw )
    text= deciconv.HRS_MIN_SEC.from_std( deciconv.HOURS.to_std( hrs ) )
...
def degrees_to_sexagesimal():
    ...
    deg= float( deg_raw )
    text= deciconv.DEG_MIN_SEC.from_std( deciconv.DEGREES.to_std( deg ) )
...
def sexagesimal_to_hours():
    ...
    hrs= deciconv.HOURS.from_std( deciconv.HRS_MIN_SEC.to_std( hms_raw ) )
...
def sexagesimal_to_degrees():
    ...
    deg= deciconv.DEGREES.from_std( deciconv.DEG_MIN_SEC.to_std( dms_raw ) )
    fmt= deciconv.DEG_MIN_SEC.from_std( deciconv.DEGREES.to_std( deg ) )
